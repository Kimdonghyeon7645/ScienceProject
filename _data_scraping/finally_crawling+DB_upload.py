...
state = input("데이터 베이스를 구축할 방법을 선택하세요\n"
              "{수동(원격으로 접속시) DB 생성: 1 /수동 DB 업데이트(기존에 생성했을시): 2 / 로컬DB 생성: 3 / 로컬DB 업데이트: 4} : ")
database_name = "pj_db"
user_name = 'bosal'


# 수동 데이터베이스를 구축할 코드 출력
# 참고로 SQL문 여러줄을 모두 글어서 붙여넣기하면, 윗줄부터 순서대로 적용이 됩니다. 한줄씩 쪼개서 소심하게 복붙안해도 되요.
if state is '1':
    # header 테이블 생성 + 삽입문 출력
    print(f"USE {user_name}${database_name}")
    # 위 코드는 pythonanywhere 에서 서버 배포를 할때 지원하는 원격 DB서버의 데이터베이스 이름입니다. 딴곳이랑 이부분만 달라요.
    print(
...
# 수동 데이터베이스 업데이트할 코드 출력
elif state is '2':
    # header 테이블 업데이트
    print(f"USE {user_name}${database_name}")
    print("UPDATE header SET ", end='')
    header_fiend = ["update_year", "update_month", "update_date", "update_time"]
    for i, yo_so in enumerate(header[1:]):
        if i is 0:
            print(header_fiend[i], " = '", yo_so, sep='', end="'")
        else:
            print(", ", header_fiend[i], " = '", yo_so, sep='', end="'")
    print(" WHERE num = ", header[0], ';', sep='')

    # 구별로 테이블 업데이트
    for index, gu_data in enumerate(all_data):
        for hour_data in gu_data:
            print(f"UPDATE gu_{index} SET")
            hour_fiend = [
                "time_difference", "hour", "temp", "day_high_temp", "day_low_temp", "sky_state", "weather_state",
                "weather_eng", "rain_persent", "expect_rain_6h", "expect_rain_12h", "expect_snow_6h", "expect_snow_12h",
                "wind_speed", "wind_way", "wind_text", "humi", "fine_dust", "small_fine_dust"
            ]
            for i, yo_so in enumerate(hour_data[1:]):
                if i is 0:
                    print(hour_fiend[i], " = '", yo_so, sep='', end="'")
                else:
                    print(", ", hour_fiend[i], " = '", yo_so, sep='', end="'")
            print(" WHERE num = ", hour_data[0], ';', sep='')
